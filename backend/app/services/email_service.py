...
async def send_test_email(to_email: str) -> dict:
    """Sends a minimal test email and returns diagnostic details."""
    subject = "TradeShift: SMTP Connectivity Test"
    content = _heading("SMTP Connectivity Test") + \
              _text("This is a test email triggered to verify your SMTP settings on Azure.") + \
              _info_table([
                  ("Server", os.getenv("MAIL_SERVER")),
                  ("Port Fallback", "587 -> 2525"),
                  ("Timestamp", str(datetime.datetime.now()))
              ]) + \
              _cta_button("System Status", "https://20.40.42.232.nip.io/health")
    
    html = _html_wrapper("Diagnostic Test", content)
    ...
    for port in ports_to_try:
        try:
            ...
            logger.info(f"✅ Diagnostic email sent via {port}")
            return {"status": "success", "port": port, "message": f"Sent successfully via {port}"}
        except Exception as e:
            err_msg = str(e)
            results.append({"port": port, "error": err_msg})
            logger.warning(f"⚠️ Diagnostic attempt failed on {port}: {err_msg}")
            
    return {"status": "error", "attempts": results}
