...
const FALLBACK_IMAGES = [
  "https://images.unsplash.com/photo-1611974717483-58eb8cd4205c?auto=format&fit=crop&q=80&w=400",
  "https://images.unsplash.com/photo-1590283603385-17ffb3a7f29f?auto=format&fit=crop&q=80&w=400",
  "https://images.unsplash.com/photo-1535320903710-d993d3d77d29?auto=format&fit=crop&q=80&w=400"
];

const NewsPanelItem = ({ item, isNew, qInputs, setQInputs, loadingQAs, handleAsk }: any) => {
    const [imgSrc, setImgSrc] = useState(item.imageUrl || FALLBACK_IMAGES[Math.floor(Math.random() * FALLBACK_IMAGES.length)]);
    ...
    return (
        <div className={`group relative p-4 rounded-md border transition-all duration-300 ${
            isNew 
            ...
            <div className="flex gap-4">
                {/* Thumbnail */}
                <div className="w-16 h-16 rounded-lg overflow-hidden shrink-0 border border-white/10 bg-slate-800">
                    <img 
                        src={imgSrc} 
                        alt="" 
                ...
                <div className="flex-1 min-w-0">
                    <h4 className="text-sm font-bold text-white leading-tight mb-2 group-hover:text-primary transition-colors line-clamp-2">
                        {item.title}
                    </h4>
                </div>
            </div>
            ...
            {item.analysis === "Analyzing impact..." ? (
                <div className="flex items-center text-[10px] text-[#2962FF] gap-1.5 animate-pulse py-2 mt-2">
                    ...
                    <div className="p-3 bg-black/40 rounded-xl border border-white/5 flex flex-col gap-2 ring-1 ring-white/5">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Sentiment Analysis</span>
                            </div>
                        ...
                        <div className="flex flex-col gap-2 mt-2 p-3 bg-gradient-to-r from-blue-500/5 to-transparent rounded-xl border border-blue-500/10 overflow-hidden relative">
                            ...
                            <div className="grid grid-cols-2 gap-4 relative z-10">
                                <div className="space-y-0.5">
                                    ...
                                    <div className="text-xs font-mono font-bold text-white">{item.predicted_impact !== "Unknown" ? item.predicted_impact : "CALC..."}</div>
                                </div>
                                <div className="space-y-0.5">
                        ...
                        <div className="mt-4 p-4 rounded-xl bg-gradient-to-br from-indigo-500/10 to-purple-500/10 border border-white/5 relative overflow-hidden group/explainer">
                            <div className="space-y-3 relative z-10">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] font-black text-white uppercase tracking-widest">Analyst Brief</span>
                                </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-white/5">
                        ...
                        {item.qa_history && item.qa_history.length > 0 && (
                            <div className="flex flex-col gap-3 mb-4 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                                {item.qa_history.map((qa: any, qId: number) => (
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-4 custom-scrollbar bg-gradient-to-b from-transparent to-black/20">
                ...
                {newsItems.map((item, i) => (
                    <NewsPanelItem 
                        ...
                        isNew={i === 0}
                        qInputs={qInputs}
                        setQInputs={setQInputs}
                        loadingQAs={loadingQAs}
                        handleAsk={handleAsk}
                    />
                ))}
            </div>
